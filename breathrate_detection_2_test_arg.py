...
from arg_GridSearch import get_arg
import math
...
def detect_breath(unw_phase, count, disp, array_arg):
    # Define 
    sampling_rate = 20
    ...
    # plt.subplot(3, 1, 1)
    plt.plot(raw_x, raw)
    # plt.legend(labels=['Unwrap phase'], loc='upper right')
    ...
    # plt.legend(labels=['Phase difference'], loc='upper right')
    ...
    for i in range(1, int(len(phase_diff))-1):
        ...
        a = filter_RemoveImpulseNoise(phase_diff, dataPrev2, dataPrev1, dataCurr, array_arg[0])
        if a > 0:
            a += 1
        elif a < 0:
            a -= 1
    ...
    # plt.legend(labels=['Remove Impulse Noise'], loc='upper right')
    ...
    bandpass_sig = iir_bandpass_filter_1(new_phase_diff, array_arg[1], array_arg[2], 20, array_arg[3], array_arg[4]) # Breath: 0.1 ~ 0.33 order=5, Hreat: 0.8 ~ 2.3
    ...
    # plt.legend(labels=['Bandpass signal'], loc='upper right')
    ...
    # Smoothing signal
    plt.figure()
    smoothing_signal = MLR(bandpass_sig, 9)  # Breath = 9, Heart = 6, Delta = 1
    ...
    # plt.legend(labels=['Smoothing signal'], loc='upper right')
    ...
    # plt.legend(labels=['Feature detect'], loc='upper right')
    ...
    # Feature compress
    plt.figure()
    compress_peak, compress_valley = feature_compress(feature_peak, feature_valley, 20, smoothing_signal)  # Br: 20 Hr: 6  ex: 25
    ...
    # plt.legend(labels=['Feature compress'], loc='upper right')
    ...
    # Candidate_search
    plt.figure()
    NT_points, NB_points = candidate_search(smoothing_signal, compress_feature, 20)  # breath = 18 hreat = 4 ex7
    ...
    # plt.legend(labels=['Candidate_search'], loc='upper right')
...
if __name__ == '__main__':
    ...
    all_loss = []
    arg_array = get_arg()
    for arg_number in tqdm(range(len(arg_array))):
        for user in os.listdir("dataset"):
            if os.path.isdir(os.path.join("dataset", user, "gt_br")):
                ...
                for tmp in (range(0, len(files)//2, 1)):
                    ...
                    heart = vitial_sig['outputFilterHeartOut'].values
                    breath = vitial_sig['outputFilterBreathOut'].values
                    for i in range (0, 800, 800):  # 0, 600, 1200
                        result_rate = detect_breath(unwrapPhase[0 + i: 800 + i], count, disp, arg_array[arg_number])
                ...
                '''
                print(f'Absolute error in dataset: {absolute_error_ans}')
                print("user")
                print("predict_array")
                print(predict_array)
                ...
                '''
        print(arg_array[arg_number])
        print("AVG L1 lOSS",calculate_l1_loss(all_gt_array, all_pr_array))
        all_loss.append(calculate_l1_loss(all_gt_array, all_pr_array))
        time_End = time.time()
        print('time:',float(time_End - time_Start))
    print("min", np.argmin(all_loss))
    print(arg_array[math.floor(np.argmax(all_loss) / len(arg_array))])