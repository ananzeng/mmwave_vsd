...
def detect_breath(unw_phase):
    ...
    # butter ellip cheby2
    bandpass_sig = iir_bandpass_filter_1(new_phase_diff, 0.9, 1.9, 20, 9, "cheby2") # 0.8 2.1 
    N = len(bandpass_sig)
    T = 1 / 20
...
if __name__ == '__main__':
    ...
    for data_type in type:
        for user in os.listdir(os.path.join("rf", data_type)):
            if os.path.isdir(os.path.join("rf", data_type, user, "gt_hr")):
                files_path = os.path.join("rf", data_type, user, "0.8")
                #print(files_path)
                ground_truth_files_path = os.path.join("rf", data_type, user, "gt_hr")
